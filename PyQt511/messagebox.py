...
class Example(QWidget):
    ...
    def warning(self):
        ...
        #上面是我们自定义的警告信息对话框，按钮是中文的，还有一个复选框。

        cb.stateChanged.connect(self.check)#复选框选中状态信号发射，这个连接到槽函数check()上。
        reply = msgBox.exec()#执行这个对话框
    ...
    def critical(self):
        ...
        msgBox.setDefaultButton(QMessageBox.Retry)#默认是QMessageBox.Retry
        msgBox.setDetailedText('这是详细的信息：学点编程吧，我爱你！')#增加详细信息
        reply = msgBox.exec()
    ...
    def about(self):
        ...
        #上面使用的是PyQt5默认的关于信息对话框

        msgBox = QMessageBox(QMessageBox.NoIcon, '关于', '不要意淫了，早点洗洗睡吧!')#注意是NoIcon，你可以换成其他试试。
        msgBox.setIconPixmap(QPixmap("beauty.png"))#对话框上增加一张美女图片
        msgBox.exec()
